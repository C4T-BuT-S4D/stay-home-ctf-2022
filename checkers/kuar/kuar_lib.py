...
class CheckMachine:
    ...
    def register(self, username: str, password: str):
        ...
        if not packet.decode().startswith("[?] Username: "):
            self.c.cquit(Status.MUMBLE, 
            "Register format error", 
            "CheckMachine.register(): Username msg is changed!"
        )
        
        self.send(username.encode())
        packet = self.recv(DEFAULT_RECV_SIZE)

        if packet.decode().startswith("[-] User exist"):
            self.c.cquit(Status.MUMBLE, 
            "Registration error", 
            "CheckMachine.register(): User already exist!"
        )
    ...
    def get_qr(self, status):
        ...
        try:
            ...
            self.c.cquit(status, "Can't decode QR-code", 
               "Exception in qr-code decode: {}, path: {}".format(err, qr_path))
        return text
